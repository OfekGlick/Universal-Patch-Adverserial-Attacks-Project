...
class Const(Attack):
    ...
    def attack_eval(self, pert, data_shape, eval_data_loader, eval_y_list, device):
        with torch.no_grad():
            ...
            [traj_pert_l0_ratio_list[idx].append(traj_pert_l0_ratio[idx]) for idx in range(data_shape[0])]
            traj_pert_l0_ratio_means = [np.mean(frame_pert_l0_ratio_list) for frame_pert_l0_ratio_list in
                                        traj_pert_l0_ratio_list]
            traj_pert_l0_ratio_stds = [np.std(frame_pert_l0_ratio_list) for frame_pert_l0_ratio_list in
                                       traj_pert_l0_ratio_list]
